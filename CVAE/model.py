...
==========================================================
Architecture:
  - ConditionEmbedder: strain, sex, study, tissue, flight → condition vector
  - Encoder:           expression + condition → μ, log_var
  - Decoder:           z + condition → NB params + flight classification
...
class ConditionEmbedder(nn.Module):
    ...
    Covariates:
        ...
        tissue:  tissue type         (categorical — biological condition)
        flight:  spaceflight label   (binary)
    ...
    Args:
        ...
        study_emb_dim:  embedding dim for study (kept small — batch variable)
        tissue_emb_dim: embedding dim for tissue (larger — biological)
        flight_emb_dim: embedding dim for spaceflight label
    """
    def __init__(
        self,
        n_strains: int,
        ...
        self.flight_emb = nn.Embedding(2,         flight_emb_dim)

        self.output_dim = (strain_emb_dim + sex_emb_dim + study_emb_dim +
                           tissue_emb_dim + flight_emb_dim)

    def forward(self, strain, sex, study, tissue, flight):
        """
        Args:
            strain: (B,) LongTensor
            sex:    (B,) LongTensor
...
class Decoder(nn.Module):
    """
    Maps (z, condition) → NB reconstruction params + classification logit.
    ...
    def __init__(
        ...
        self.cls_head   = nn.Linear(in_dim, 1)         # spaceflight logit
...
class SpaceflightCVAE(nn.Module):
    ...
    def forward(self, x, strain, sex, study, tissue, flight):
        """
        Args:
            x:       (B, n_genes)
            strain:  (B,) LongTensor
        ...
        """
        cond = self.embedder(strain, sex, study, tissue, flight)
        mu, log_var = self.encoder(x, cond)
        z = self.reparameterize(mu, log_var)
        log_r, p, flight_logit = self.decoder(z, cond)
    ...
    @torch.no_grad()
    def encode(self, x, strain, sex, study, tissue, flight):
        """Return μ deterministically (no sampling)."""
        cond = self.embedder(strain, sex, study, tissue, flight)
        mu, _ = self.encoder(x, cond)
        return mu

    @torch.no_grad()
    def generate(self, z, strain, sex, study, tissue, flight):
        """Decode z back to expression space."""
        cond = self.embedder(strain, sex, study, tissue, flight)
        log_r, p, _ = self.decoder(z, cond)
        return log_r, p
