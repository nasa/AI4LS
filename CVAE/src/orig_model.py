...
Key design decisions:
  1. Flight excluded from condition embedder — classifier on z only.
     Forces encoder to encode spaceflight into the latent space.
  2. All condition embedding dimensions are configurable via CLI arguments,
     allowing experimentation with different latent/condition dim ratios.

Default condition dims (total 76):
  tissue(32) + strain(16) + sex(4) + study(16) + euth(8)

Example reduced condition dims (total 28):
  tissue(16) + strain(4) + sex(2) + study(4) + euth(2)
"""
...
class ConditionEmbedder(nn.Module):
    ...
    Covariates (flight intentionally excluded):
    ...
    All embedding dimensions are configurable to allow tuning the
    condition vs latent space capacity ratio.
...
class SpaceflightCVAE(nn.Module):
    ...
    Flight is excluded from the condition embedder. A standalone
    classification head reads directly from z, forcing the encoder to
    encode the spaceflight signal into the latent space.

    All condition embedding dimensions are configurable, allowing
    experimentation with different condition vs latent dim ratios.
    ...
    def __init__(
        ...
        # Classification head reads from z only
        ...
        print(f"  Encoder input:  {n_genes} genes + {cond_dim} cond = {n_genes + cond_dim}")
        print(f"  Latent dim:     {latent_dim}")
        print(f"  Decoder input:  {latent_dim} latent + {cond_dim} cond = {latent_dim + cond_dim}")
        print(f"  Cond breakdown: tissue={tissue_emb_dim} strain={strain_emb_dim} "
              f"sex={sex_emb_dim} study={study_emb_dim} euth={euth_emb_dim} "
              f"total={cond_dim}")
    ...
    def forward(self, x, strain, sex, study, tissue, euth, flight):
        """
        Full forward pass.
        flight is accepted for API compatibility but NOT passed to the
        condition embedder — only used as the classification target.

        Returns dict: mu, log_var, z, log_r, p, flight_logit, study_logit
        """
        cond = self.embedder(strain, sex, study, tissue, euth)
        mu, log_var = self.encoder(x, cond)
        z = self.reparameterize(mu, log_var)
        log_r, p = self.decoder(z, cond)
        flight_logit = self.cls_head(z)
