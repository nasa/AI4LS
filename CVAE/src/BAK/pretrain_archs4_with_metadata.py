...
class ARCHS4Dataset(Dataset):
    ...
    def __init__(self, h5_path, normalize=True):
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            tissue_raw = np.array([decode(v) for v in f["meta/samples/tissue"][:]])
            #strain_raw = np.array([decode(v) for v in f["meta/samples/strain"][:]])
            #sex_raw    = np.array([decode(v) for v in f["meta/samples/sex"][:]])
        ...
        # no spaceflight labels in pretrain
        self.flight = np.full(len(expr), -1, dtype=np.int64)
        ...
        #print(f"  Strains:  {self.n_strains}")
        #print(f"  Sexes:    {self.n_sexes}")
    ...
    def __getitem__(self, idx):
        return {
            ...
            # pretrain placeholders — not used in loss
            #"study":  torch.tensor(0, dtype=torch.long),
            #"euth":   torch.tensor(0, dtype=torch.long),
            #"flight": torch.tensor(0, dtype=torch.long),
        }
...
class PretrainCVAE(nn.Module):
    """
    Stripped-down cVAE for pretraining.
    No flight embedding, no classification head, no study/euth embeddings.
    Only tissue, strain, sex as conditions.

    After pretraining, encoder and decoder weights are transferred
    to SpaceflightCVAE for fine-tuning.
    """

    #def __init__(self, n_genes, n_tissues, n_strains, n_sexes, latent_dim=64, hidden_dims=None, dropout=0.2):
    def __init__(self, n_genes, n_tissues, latent_dim=64, hidden_dims=None, dropout=0.2):
        super().__init__()
        if hidden_dims is None:
            hidden_dims = [256, 128]

        # condition embeddings (subset of full model)
        from torch import nn
        self.tissue_emb = nn.Embedding(n_tissues, 32)
        #self.strain_emb = nn.Embedding(n_strains, 16)
        #self.sex_emb    = nn.Embedding(n_sexes,   4)
        #cond_dim = 32 + 16 + 4   # 52
        cond_dim = 32
    ...
    #def encode(self, x, tissue, strain, sex):
    def encode(self, x, tissue):
        cond = torch.cat([
    ...
    def decode(self, z, cond):
        h     = self.decoder_net(torch.cat([z, cond], dim=-1))
        log_r = self.log_r_head(h)
        p     = torch.sigmoid(self.p_head(h))
        return log_r, p

    #def forward(self, x, tissue, strain, sex):
    def forward(self, x, tissue):
        mu, log_var, cond = self.encode(x, tissue)
        std = torch.exp(0.5 * log_var)
        z   = mu + std * torch.randn_like(std)
        log_r, p = self.decode(z, cond)
...
def run_pretrain_epoch(model, loader, optimizer, device, kl_weight, training=True):
    ...
    with torch.set_grad_enabled(training):
        for batch in loader:
            ...
            #outputs = model(x, tissue, strain, sex)
            outputs = model(x, tissue)
