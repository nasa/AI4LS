...
Checkpoint naming convention (used by hpsearch.py to match pretrain to finetune):
  <output_dir>/pretrain_best.pt
  output_dir should encode architecture, e.g.:
    checkpoints/pretrain/ld64_hd256-128_tissue/

Usage:
    python pretrain_archs4.py \\
        --data archs4_pretrain.h5 \\
        --output_dir checkpoints/pretrain/ld64_hd256-128_tissue/ \\
        --conditions tissue \\
...
def pretrain(args):
    ...
    model = SpaceflightCVAE(
        ...
        hidden_dims=args.hidden_dims,      # now configurable
        ...
        grl_alpha=0.0,                     # no adversarial during pretrain
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if va_loss < best_val:
            ...
            torch.save({
                ...
                "n_genes":      dataset.n_genes,
                "latent_dim":   args.latent_dim,
                "hidden_dims":  args.hidden_dims,   # saved for matching in hpsearch
    ...
    print(f"      --hidden_dims {' '.join(str(d) for d in args.hidden_dims)} \\")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_dims",      type=int,   nargs="+", default=[256, 128],
                        help="Encoder/decoder hidden layer sizes (default: 256 128)")
