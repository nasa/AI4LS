...
def train(args):
    ...
    if args.pretrain_checkpoint:
        ...
        # transfer only encoder and decoder weights — skip condition embeddings
        # (pretrain had different condition space: no study, euth, flight)
        transferred = []
        skipped     = []
        for key, val in pretrain_state.items():
            # map pretrain keys to SpaceflightCVAE keys
            # PretrainCVAE uses encoder_net/mu_head/logvar_head/decoder_net
            # SpaceflightCVAE uses encoder.net/encoder.mu/encoder.logvar/decoder.net
            mapped_key = None
            ...
            # skip condition embeddings — different vocab sizes
            else:
                skipped.append(key)
                continue
    ...
    # --- Optimizer & scheduler ---
    # use lower LR when fine-tuning from pretrained weights
    lr = args.lr if not args.pretrain_checkpoint else min(args.lr, 1e-4)
