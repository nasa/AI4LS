...
def train(args):
    ...
    if args.pretrain_checkpoint:
        ...
        for key, val in pretrain_state.items():
            # skip condition embeddings and classifier/discriminator heads
            # — these are randomly initialized and learned from GeneLab data
            if any(key.startswith(p) for p in [
                "embedder.", "cls_head.", "batch_disc."
            ]):
                skipped.append(key)
                continue

            # handle both old PretrainCVAE key names and new SpaceflightCVAE key names
            if key.startswith("encoder_net."):
                ...
                # key already uses SpaceflightCVAE naming — use directly
                mapped_key = key
        ...
        # optionally re-initialize latent projection heads
        # this replicates the accidental advantage of tissue_v3 where the
        # mu/logvar heads were randomly re-initialized due to a latent_dim
        # mismatch — forcing the encoder to learn a fresh latent organization
        # from GeneLab data while preserving pretrained encoder body weights
