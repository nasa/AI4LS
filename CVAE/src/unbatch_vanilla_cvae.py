...
class Unbatch_Vanilla_CVAE(torch.nn.Module):
    def __init__(self, n_genes, n_labels, latent_size, **kwargs):
        super(Unbatch_Vanilla_CVAE, self).__init__()
        ...
        self.mean = nn.Sequential(
            nn.Linear(400, self.latent_size),
            nn.Dropout(self.dropout))
        ...
        self.decoder = nn.Sequential(
            nn.Linear(self.latent_size + self.n_labels, 400),
            nn.BatchNorm1d(400),
            nn.ReLU(),
            nn.Dropout(self.dropout),
            nn.Linear(400, 800),
    ...
    def train_net(self, train_loader, n_epochs = 100, patience = 10, 
        ...
        while epoch < n_epochs and count_to_patience < patience:
            ...
            epoch_loss = loss_val / (len(train_loader) * train_loader.batch_size)
