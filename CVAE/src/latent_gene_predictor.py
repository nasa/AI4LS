...
def load_model(checkpoint_path, dataset, device="cpu"):
    ...
    # Read hidden_dims from checkpoint if available (updated pretrain/train scripts
    # save this); fall back to [256, 128] for older checkpoints.
    hidden_dims = ckpt.get("hidden_dims", ckpt.get("args", {}).get("hidden_dims", [256, 128]))
    ...
    print(f"  conditions={arch['conditions']}  latent_dim={arch['latent_dim']}  hidden_dims={hidden_dims}")
...
def compute_gene_scores(X, z, flight_dims, corrs, method="lasso",
                        min_samples=20, max_iter=50000, tol=1e-3):
    """
    ...
    Args:
        ...
        max_iter:    maximum coordinate descent iterations (default 50000)
        tol:         convergence tolerance (default 1e-3, relaxed from 1e-4)
    ...
    # Scale features once — critical for Lasso convergence with 18k genes
    ...
    # Suppress ConvergenceWarning after logging it once per dim
    n_converge_warnings = 0

    for i, d in enumerate(flight_dims):
        z_d    = z[:, d]
        # Also scale the target per-dim for better numerical conditioning
        z_scaler = StandardScaler()
        z_d_sc   = z_scaler.fit_transform(z_d.reshape(-1, 1)).ravel()
        weight   = np.abs(corrs[d])

        print(f"  Fitting dim z{d:02d} (r={corrs[d]:+.3f}, weight={weight:.3f}) "
              f"[{i+1}/{len(flight_dims)}]...")

        if method == "lasso":
            reg = LassoCV(
                cv=5,
                max_iter=max_iter,   # 50k default vs original 5k
                tol=tol,             # 1e-3 default vs sklearn's 1e-4
                n_alphas=30,
                random_state=42,
                n_jobs=-1,
            )
        ...
        # Catch any remaining convergence warnings and report them clearly
        # rather than spamming the output
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always", ConvergenceWarning)
            reg.fit(X_sc, z_d_sc)
            for w in caught:
                if issubclass(w.category, ConvergenceWarning):
                    n_converge_warnings += 1
                    if n_converge_warnings == 1:
                        print(f"    [warn] Convergence warning on z{d:02d} — "
                              f"duality gap not fully closed. "
                              f"Results still usable; try --max_iter higher "
                              f"or --tol looser if this appears frequently.")

        coef = reg.coef_   # (n_genes,)
        n_nonzero = (coef != 0).sum()
        print(f"    alpha={reg.alpha_:.4f}  nonzero_genes={n_nonzero}")
    ...
    if n_converge_warnings > 0:
        print(f"\n  [info] {n_converge_warnings} convergence warning(s) total. "
              f"Try --max_iter 100000 or --tol 1e-2 to reduce further.")
...
def main():
    ...
    # Convergence controls — exposed as CLI args so you can tune without
    # editing the script
    parser.add_argument("--max_iter",        type=int, default=50000,
                        help="Max Lasso coordinate descent iterations "
                             "(default 50000; increase if warnings persist)")
    parser.add_argument("--tol",             type=float, default=1e-3,
                        help="Lasso convergence tolerance "
                             "(default 1e-3; try 1e-2 if warnings persist)")
