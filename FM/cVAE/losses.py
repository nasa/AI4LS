...
class CVAELoss(nn.Module):
    ...
    def forward(
        ...
        study: torch.Tensor,
        kl_weight: float = None,   # override beta for annealing
    ) -> dict:
        """
        Args:
            outputs:   dict returned by SpaceflightCVAE.forward()
            x_raw:     (B, G) raw integer counts (not log-transformed)
            flight:    (B,) LongTensor spaceflight labels
            duration:  (B,) FloatTensor mission durations
        ...
        l_reg   = regression_loss(outputs["duration_hat"], duration)
