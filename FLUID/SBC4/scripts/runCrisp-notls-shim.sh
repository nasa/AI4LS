...
update_plans_iss() {
	...
	cp /mnt/crisp_task_runner.py ~/crisp/fl_src
	cp /mnt/erm_module.py ~/crisp/fl_src
	cp /mnt/irm_module.py ~/crisp/fl_src
...
update_plans_moon() {
	ROLE=$1
	# update plan.yaml
	/usr/bin/sed -i "s/.*agg_addr.*/    agg_addr: $AGG_MOON_HOST/" ~/crisp/fl_plan/plan.yaml
	/usr/bin/sed -i "s/.*agg_port.*/    agg_port: $AGG_MOON_PORT/" ~/crisp/fl_plan/plan.yaml
	/usr/bin/sed -i "s/.*disable_client_auth.*/    disable_client_auth: true/" ~/crisp/fl_plan/plan.yaml
	/usr/bin/sed -i 's/.*disable_tls.*/    disable_tls: true/' ~/crisp/fl_plan/plan.yaml
	/usr/bin/sed -i "s/.*collaborator_count.*/    collaborator_count: $COLABORATOR_COUNT/" ~/crisp/fl_plan/plan.yaml
	#/usr/bin/sed -i "s/.*collaborator_count.*/    collaborator_count: 1/" ~/crisp/fl_plan/plan.yaml
	#/usr/bin/sed -i "s/.*best_state_path.*/    best_state_path: $WORKSPACE_MOON_AGG_DIR\/crisp_best_.pbuf/" ~/crisp/fl_plan/plan.yaml
	#/usr/bin/sed -i "s/.*init_state_path.*/    init_state_path: $WORKSPACE_MOON_AGG_DIR\/crisp_init_.pbuf/" ~/crisp/fl_plan/plan.yaml
	#/usr/bin/sed -i "s/.*last_state_path.*/    last_state_path: $WORKSPACE_MOON_AGG_DIR\/crisp_last_.pbuf/" ~/crisp/fl_plan/plan.yaml
	/usr/bin/sed -i "s/.*rounds_to_train.*/    rounds_to_train: $ROUNDS/" ~/crisp/fl_plan/plan.yaml
        /usr/bin/sed -i "s/.*db_store_rounds.*/    db_store_rounds: $ROUNDS/" ~/crisp/fl_plan/plan.yaml
        /usr/bin/sed -i "s/.*num_epochs.*/      num_epochs: $EPOCHS/" ~/crisp/fl_plan/plan.yaml
	...
	if [ "$ROLE" == "agg-moon" ]
        then
                cd $WORKSPACE_MOON_AGG_DIR
        elif [ "$ROLE" == "colab-moon" ]
	then
                cd $WORKSPACE_MOON_COLAB_DIR
	else
		echo wrong role:  $ROLE
		exit 1
        fi
	...
	cp -r ~/crisp/fl_plan/* plan/
	cp -r ~/crisp/fl_src/* src/
	fx plan initialize -a $AGG_MOON_HOST
}
...
update_plans_earth() {
	...
	then
		cd $WORKSPACE_ISS_COLAB_SHIM_DIR
	elif [ "$ROLE" == "colab-moon" ]
	then
		cd $WORKSPACE_MOON_COLAB_SHIM_DIR
	else
		echo wrong role:  $ROLE
		exit 1
        fi
...
run_agg_moon() {
	ROLE=agg-moon
	if [ -d $WORKSPACE_MOON_AGG_DIR ]
	then
		rm -rf $WORKSPACE_MOON_AGG_DIR
	fi
	mkdir -p $WORKSPACE_MOON_AGG_DIR
	update_plans_moon agg-moon 
	fx aggregator shim
}
...
run_colab_shim_iss() {
	ROLE=colab-iss
	if [ -d $WORKSPACE_ISS_COLAB_SHIM_DIR ]
	then
		rm -rf $WORKSPACE_ISS_COLAB_SHIM_DIR
	fi
	mkdir -p $WORKSPACE_ISS_COLAB_SHIM_DIR
	update_plans_earth $ROLE 
	mkdir $WORKSPACE_ISS_COLAB_SHIM_DIR/workspace/proto_path
	fx collaborator shim  -n $ROLE -p plan/plan.yaml -d plan/data.yaml
}

run_colab_shim_moon() {
	ROLE=colab-moon
	if [ -d $WORKSPACE_MOON_COLAB_SHIM_DIR ]
	then
		rm -rf $WORKSPACE_MOON_COLAB_SHIM_DIR
	fi
	mkdir -p $WORKSPACE_MOON_COLAB_SHIM_DIR
	update_plans_earth $ROLE 
	mkdir $WORKSPACE_MOON_COLAB_SHIM_DIR/workspace/proto_path
	fx collaborator shim  -n $ROLE -p plan/plan.yaml -d plan/data.yaml
}
...
run_colab_iss() {
	...
	mkdir -p $WORKSPACE_ISS_COLAB_DIR
	update_plans_iss $ROLE 
	fx collaborator start  -n $ROLE -p plan/plan.yaml -d plan/data.yaml
}

run_colab_moon() {
	ROLE=colab-moon
	if [ -d $WORKSPACE_MOON_COLAB_DIR ]
	then
		rm -rf $WORKSPACE_MOON_COLAB_DIR
	fi
	mkdir -p $WORKSPACE_MOON_COLAB_DIR
	update_plans_moon $ROLE 
	fx collaborator start  -n $ROLE -p plan/plan.yaml -d plan/data.yaml
}
