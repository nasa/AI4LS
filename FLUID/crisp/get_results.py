...
def parse_args():
	...
	parser.add_argument('-nf', '--numFeatures', help='number of features', default=None, required=False)
...
def main():
	...
	print('num coefs: ', len(coefficients))
	print('features: ', features)
	...
	#print(featureDict)	
	# Sort keys based on values in ascending order
	sorted_keys = sorted(featureDict, key=featureDict.get, reverse=True)
	#sorted_dict = dict(sorted(featureDict.items(), key=lambda item: item[1]))
