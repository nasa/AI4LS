...
def get_datasets(dataframe_fp, subject_keys, test_val_split, predictors, targets, environments, exclude, remove_keys):
    ...
    for val in np.unique(train[environments].values):
        env_ds = DataFrameDataset(dataframe=grouped.get_group(val), 
                                  predictor_columns=predictors, 
                                  target_columns=targets, exclude_columns=exclude)
...
def get_datasets_for_experiment(config):
    ...
    if data_config["synthetic_train_test_split"]:
        if config["verbose"]:
            print("Using synthetic dataset train/test split")   
            _, _, test_dataset = get_datasets(
                dataframe_fp=data_config['dataset_fp_test'],
        ...
        if data_config.get("dataset_fp_val") is None:
            environment_datasets, val_dataset, _ = get_datasets(
                dataframe_fp=data_config['dataset_fp_train'],
                subject_keys=data_config['subject_keys'],
                test_val_split=(0.,0.0), # 0.2 fraction of train for val
                predictors=data_config['predictors'],
                targets=data_config['targets'],
                environments=data_config['environments'],
                exclude=data_config.get('exclude', []),
            ...
            environment_datasets, _, _ = get_datasets(
                dataframe_fp=data_config['dataset_fp_train'],
                subject_keys=data_config['subject_keys'],
                test_val_split=(0.,0.),
                predictors=data_config['predictors'],
            ...
            _, val_dataset, _ = get_datasets(
                dataframe_fp=data_config['dataset_fp_val'],
                subject_keys=data_config['subject_keys'],
                test_val_split=(0.,1.),
                predictors=data_config['predictors'],
                targets=data_config['targets'],
    ...
    if config['verbose']:
        ...
        if data_config.get("dataset_fp_val") is not None:
