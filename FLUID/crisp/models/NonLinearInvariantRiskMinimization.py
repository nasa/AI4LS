...
class NonLinearInvariantRiskMinimization(object):
    ...
    def train(self):
        ...
        for step in range(self.args['n_iterations']):
            ...
            env_accs = []
            env_pens = []
            for env_loader in self.train_loaders: #outer loop goes through each environment
                env_logits = []
                env_targets = []
                for inputs, targets in env_loader: #inner loop goes through each inout
                    if self.cuda:
                        inputs = inputs.cuda()
                        env_logits.append(self.model(inputs).cpu().unsqueeze(-1))
                    else:
                        e_log = self.model(inputs).unsqueeze(-1).unsqueeze(0) #the self-model supposedly corresponds to phi in the paper, but where is the W
