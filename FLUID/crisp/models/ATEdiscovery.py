...
WARNING: model deprecated and not used anymore

"""

class ATE(object):
    def __init__(self, train_environments, val_environment, test_environment, args):
        print('Warning: model deprecated.')
        ...
        for i in range(d):
            ...
            ## Check if outcome and treatment variables are roughly balanced 
            ...
            ATE[i, 1] = treatment_coef
            ATE[i, 2] = treatment_pval

        self.ATEdf = pd.DataFrame(ATE, columns=['index', 'effect', 'significance'])
    ...
    def results(self):
        outputdf = self.ATEdf.copy()
        outputdf = outputdf.dropna()
        dropped = 0
        for elem in outputdf['index']:
            print(self.ATEdf['significance'][elem])
            if self.ATEdf['significance'][elem] > self.pvalthreshold:
