...
from models.TorchModelZoo import TorchLogisticRegressionModule, TorchLinearRegressionModule
...
class TorchMultiClassDeconfounder(object):
    """
    ...
    def __init__(self, train_environments, val_environment, test_environment, args):
        ...
        self.output_data_regime = args["output_data_regime"]
...
#             if P-values are computed the computation takes a lot longer. hence a shorter list is used
#             this is fully suitable for paralelisation
        else:
            ...
            while (not self.latent_representation_found) and (idx_std < len(std_list)):
                ...
                while (not self.latent_representation_found) and (idx_model < len(factor_model_list)):
                    ...
                    if (pval > self.minP) and (pval < self.maxP):
                        ...
                        self.latent_variables = Zhat #in this code latent variables are generated all in one go - might not be best 😞 
    ...
    def fitAssignmentModel(self, df_train, df_val, df_test, latent_dim, stddv_datapoints, modeltype, seed):
        ...
        for i in range(n_rep):
            ...
            if np.isnan(w_sample).any():
                break
'''odhran - this block is creating a sample for the X - generated. If you can generate a sample with pyro it can be removed'''
        ...
        if model == 'linearPlain':
            if self.output_pvals:
                ...
                if self.output_data_regime == "real-valued":
                    dcfresult = TorchLinearRegressionModule(dcfX_train.shape[1], 1, bias=True)
                else:
                    dcfresult = TorchLogisticRegressionModule(dcfX_train.shape[1], len(np.unique(yTrain)), bias=True, l1=_lambda)
                dcfresult.fit(dcfX_train, yTrain)
                
                coef = dcfresult.coef_()
                if coef.shape[0] == 1:
#                     print("IF TRIGGERED", coef.shape)
...
#                     print("ELSE TRIGGERED")
                    dcf_coefs = np.vstack(
                        (dcfresult.coef_(), np.zeros(shape=np.shape(dcfresult.coef_())))).T
#                     print("dcf_coefs.shape:", dcf_coefs.shape)
                    num_features = coef.shape[1]
                    
                # make predictions with the causal model 
                dcfX_val = XVal
                
                dcfy_pred = dcfresult.predict(dcfX_val).numpy()

            # Evaluate test accuracy
            if self.output_data_regime == "real-valued":
                dcf_acc = calculate_r2(yVal, dcfy_pred)
            else:
                dcf_acc = calculate_acc(yVal, dcfy_pred)
            print('predictions:', dcfy_pred.shape)
            print('truth:', yVal.shape)
            print('Acc/r_squared:', dcf_acc)


        modelOut = {}
        print('dcf_coefs:', dcf_coefs)
        colnames = ["value"]  
        colnames = colnames + ["value_"+str(i) for i in range(coef.shape[0])[1:]]
        colnames = colnames + ["pval"]
        colnames = colnames + ["pval_" + str(i) for i in range(coef.shape[0])[1:]]
        #modelOut['Features'] = pd.DataFrame(dcf_coefs, index=columnnames[index_colnames], columns=['value', 'pval'])
        modelOut["Features"] = pd.DataFrame(dcf_coefs, index=columnnames, columns=colnames)
