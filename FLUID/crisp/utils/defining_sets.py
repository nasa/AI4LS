...
def defining_sets(accepted_subsets):
    ...
    if len(accepted_subsets):
        # add all singletons (sets of dimension one)
        singletons = []
        ...
        if len(singletons) > 0:
            for i in range(len(singletons)):
                ...
                # Remove singleton list from accepted_subset (and any superset of the singleton?)
        ...
        if len(accepted_subsets) > 0:
            ...
            for i in range(len(tree_list)):
                ...
                cont = True
                cc = 0
                cc_limit = 2 # used to be 5
                while cont and cc < cc_limit:
                    #print('#########CC',cc)
...
def get_minimal_subsets(sets):
    ...
    for s in sets:
        if not any(minimal_subset.issubset(s) for minimal_subset in minimal_subsets):
            ...
            # s is a python set and we require python lists 
