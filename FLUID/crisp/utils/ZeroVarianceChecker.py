...
class ZeroVarianceChecker(object):
    def __init__(self, environment_datasets, val_dataset, test_dataset, args):
        ...
        self.threshold = args.get('variance_threshold', 0)
        ...
        if self.in_each_env:
            ...
            zero_var_cols = self.check_vars_for_df(all_df, self.threshold)
            self.zero_var_cols = zero_var_cols
    ...
    @staticmethod
    def check_vars_for_df(df, threshold=0):
        print('variance threshold is ' + str(threshold))
        zero_var_cols = []
        for col in df.columns:
            if np.std(df[col].values) <= threshold:
    ...
    def check_vars_in_each_env(self, envs):
        zero_var_sets = []
        for df in envs:
            zero_var_cols = self.check_vars_for_df(df,self.threshold)
