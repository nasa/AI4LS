...
def get_test_accuracy_figure(results_dict):
    ...
    for model in results_dict:
        model_acc = model_acc + [model[seed]['test_acc'] for seed in model]
        model_names = model_names + [model[seed]['method'] for seed in model]
            
#         if res_dict.get('test_acc', False):
#             model_names.append(res_dict['method'])
#             model_errors.append(res_dict['test_acc'])

    df = pd.DataFrame({'method': model_names, 'accuracy': model_acc})
...
def new_get_method_intersections(frame, top_k=50):
    # Collect ranked (sorted) features and weights into dataframes for each method
    reduced_frames = [frame[column].sort_values(ascending=False).iloc[:top_k] for column in frame.columns]
    method_names = [series.name for series in reduced_frames]
    method_dictionary = {series.name : set(series.index.to_list()) for series in reduced_frames}
    print(method_dictionary)
    method_intersections = pd.DataFrame(columns=method_names, index=method_names)
    
    for column in method_intersections.columns:
        for index in method_intersections.index:
            if column == index:
                method_intersections[column][index] = '-'
            else:
                method_intersections[column][index] = ' '.join(list(method_dictionary[column].intersection(method_dictionary[index])))
    
    return method_intersections
...
def new_get_corr_matrix(average_voting_ensemeble):
    import plotly.graph_objects as go
