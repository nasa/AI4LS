...
parser.add_argument('--save_dir', default='results/validation_bucket', help='directory where the plots are saved', type=str)
...
def CDF_successful_causals_links(results, causalnex=False):
#     print(results)
    CDF = [0]
    for j in range(len(results)):
        if causalnex:
            if 'Causal' in results[j][1]:
                CDF.append(CDF[-1]+1)
            else:
                CDF.append(CDF[-1])
        else:
...
#     print(results)
    ...
    for j in range(len(results)):
        if causalnex:
            if 'Causal' in results[j][1] or 'Non' in results[j][1]:
                CDF.append(CDF[-1]+1)
            else:
                CDF.append(CDF[-1])
        else:
...
def plot_variance(xx,results, index, label, percentiles = [5,25,50,75,95], CI=True):
    ...
    plt.plot(xx, rsDist[half,:],color=colors[index], label=label)
    if CI:
        for i in range(half):
            plt.fill_between(xx, rsDist[i,:],rsDist[-(i+1),:],color=colors[index], alpha=(i/half))


list_examples = ['Example1']#,'Example2','Example3','Example4',
#                 'Example5','Example_Confounder', 'Example_Nonlinear']

list_methods = ['LICP','NLICP','IRM','DCF','NLIRM']

cwd = os. getcwd()

#     for example in list_examples:

#         # Let's upload all of the results for the different methods
#         list_results = []
#         list_methods_used = []
#         for method in list_methods:
#             # search for the result
#             save_dir = cwd + '/results/validation/'+method+'_'+example+'.json'
#             if os.path.isfile(save_dir):
#                 list_methods_used.append(method)
#                 list_results.append(statistic_on_results(
#                             json.load(open(save_dir,'r'))))

#         if len(list_results)>0:
#             print('saving the results')
#             fig, ax = plt.subplots()
#             ax.set_title('Accuracy on the Predicted Causes')
#             ax.boxplot(list_results, labels=list_methods_used)
#             fig.savefig(cwd + '/results/validation/Comparison_on_'+example+'.pdf')


for example in list_examples: 
    list_methods = ['IRM', 'ERM', 'CSNX', 'NLIRM', 'CSNX_ENV', 'IRM_fed']
    list_results = []
    ...
    experiment_name = example +'_dim_inv_'+str(args.dim_inv)+\
                ...
                '_n_env_'+str(args.n_env) + '_n_seeds_' + str(args.n_seeds) +'/'
    ...
    for method in list_methods:
        ...
        if os.path.isfile(save_dir):
            ...
            CDFs_linked = []
            for key in keys:
                sorted_results = sorted(zip(results[key]["to_bucket"]["features"], results[key]["to_bucket"]["coefficients"]), key=lambda x: abs(x[1]), reverse=True)
