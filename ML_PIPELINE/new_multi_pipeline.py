...
# Local copy of TISSUE_REGISTRY (for reference only, actual data is in service)
TISSUE_REGISTRY = {
    'muscle': ['48', '51', '71', '97'],
    'bone': ['179', '180', '181'],
...
def combine_and_run_pipeline(
    ...
    **kwargs
    ...
    if not tissue_name and not osd_ids:
        raise ValueError("Either --tissue or --osd_ids must be specified")
    
    if tissue_name and osd_ids:
        raise ValueError("Cannot specify both --tissue and --osd_ids")
    ...
    try:
        ...
        dataset_path = Path("./datasets") / f"{combined_dataset_id}.parquet"
        if not dataset_path.exists():
            raise ValueError(f"Combined dataset not found at {dataset_path}")
    ...
    try:
        ...
        dataset_path = Path("./datasets") / f"{filtered_dataset_id}.parquet"
...
def get_data_client():
    """Load DataServiceClient from orchestration_service"""
...
def run_kegg_analysis(feature_importance_response, organism="mmu", pvalue_cutoff=0.05, qvalue_cutoff=0.2, max_genes=500, method=None, min_importance=0.0):
    ...
    logger.info(f"using system path: {_bio_path} for imports")
    try:
...
def run_ensemble_pipeline(dataset_id, target_column, factor_values, 
    ...
    from src.consensus import compute_consensus_features
    sys.path.remove(str(ml_service_path))
