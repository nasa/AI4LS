...
This module should be placed in: data_service/src/multi_dataset_combiner.py
"""
...
class MultiDatasetCombiner:
    ...
    def download_multiple_datasets(
        ...
        for osd_id in osd_ids:
            ...
            try:
                ...
                print(f"DEBUG - Checking: {dataset_path.absolute()}")
    ...
    def find_common_genes(self, datasets: Dict[str, pd.DataFrame]) -> pd.Index:
        ...
        # Get all gene sets
        gene_sets = []
        logger.info(f"datasets data type: {type(datasets)}")
        for osd_id, df in datasets.items():
            logger.info(f"osd_id: {osd_id}")
    ...
    def combine_datasets(
        ...
        if common_genes is None:
            logger.info("no common genes - rerunning find_common_genes ????...")
