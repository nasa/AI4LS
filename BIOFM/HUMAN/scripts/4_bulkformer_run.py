...
def main():
    ...
    t_start = time.time()
    '''esm2_data = torch.load("../BIOFM_PY/data/embeddings/esm2_t6_8M_UR50D_gene_embeddings.pt", map_location="cpu")
    esm2_raw = esm2_data['embeddings'].float().to(device)
    t_load = time.time() - t_start
    if is_main:
        print(f"  ✓ Shape: {esm2_raw.shape}, Time: {t_load:.2f}s")'''
    esm3_data = torch.load("./data/embeddings/esm3_sm_open_v1_gene_embeddings.pt", map_location="cpu")
    esm3_raw = esm3_data['embeddings'].float().to(device)
    t_load = time.time() - t_start
    if is_main:
        print(f"  ✓ Shape: {esm3_raw.shape}, Time: {t_load:.2f}s")
    ...
    model_params["gene_emb"] = esm3_raw
